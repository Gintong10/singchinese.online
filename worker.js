...
async function handleTranslationCache(request, env) {
  ...
  if (request.method === "PUT") {
    ...
    if (!payload) {
      return noStoreJson({ error: "Invalid translation cache payload." }, { status: 400 });
    }
...
export default {
  async fetch(request, env) {
    const url = new URL(request.url);
    if (url.pathname === "/api/translation-cache") {
      return handleTranslationCache(request, env);
    }
